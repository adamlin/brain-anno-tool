/* RIKEN, CSHL - openlayer viewer */
/**
 * @author Adam Lin | yarycold@gmail.com
 * @license MIT license
 **/
app.UUID = (function() {
  var self = {};
...
$(function() {
    ...
    if(brain_id == 'm852' || brain_id == 'm820' || brain_id == 'm821' || brain_id == 'm921' || brain_id == 'm1228'){
        
        app.rft_id2 = app.rft_id.replace(/JP2/g, "Mask");
    ...
    jsrc.getImageMetadata(function() {
        ...
        if (brain_id.includes('m')){
            show_points_marmoset();
        }else{
            show_points();
        }
        inj_volume = show_inj_volume();
        var delids = [];
        ...
        var getsectionnumber = pid.split('_');

        if (brain_id.includes('m')){
            getsectionnumber = getsectionnumber[2].split('.');
            getsectionnumber = parseInt(getsectionnumber[0]);
        }else{
            getsectionnumber = getsectionnumber[3].split('.');
            getsectionnumber = parseInt(getsectionnumber[0]); 
        }
        


        if(brain_id.includes('MD') || brain_id.includes('HUA')|| brain_id.includes('PTM')){
            var geourl = hostname + "atlas_data/" + brain_id + "/atlas_" + brain_id + "_" + getsectionnumber + ".json";
        }else{
            var geourl = hostname + "atlas_data/PMD" + brain_id + "/atlas_PMD" + brain_id + "_" + getsectionnumber + ".json";
        }
        ...
        atlaslayer.setVisible(false);

        $('#atlas_on_off_click').click(function (evt){
            if(this.textContent == '+'){
                checked = true;
                $('#atlas_on_off_click').text('-');
            }else{ 
                checked = false;
                $('#atlas_on_off_click').text('+');
            }
     ...
     //    app.map2 = new ol.Map({
        ...
        $('#Toggle4').change(function (evt){
            checked= evt.target.checked;
            vector_inj_volume.setVisible(checked); 
        });
        ...
        function addMarkingInteraction(){
            ...
            if($('#cellcount_onoffswitch:checkbox:checked').length == 1){ // indicated unlock
                swal({
                  title: "Lock ON/OFF !",
                  text: "The lock is off, please inseart your name",
                  type: "input",
                  ...
                  if (inputValue === "") {
                    swal.showInputError("Please insert your name for annotating this section");
                    return false
                  }

                  var filename = getParameterByName('pid').replace('%26', '&', getParameterByName('pid'));
                  // foucs to lock the
                  ...
                  regquery3 = '{"brainno": ' + br_no + ',"sliceno":"' + slice_no + '","filename":"' + filename + '", "lock": 1' + ',"datetime":"' + currentTime + '"}';
                    ...
                    // $.ajax({
                    //     type : "POST",
                    //     url  : "/mamo/get_auth_lockedinfo.php",
                    //     data: {'qry' :regquery3},
                    ...
                    //     },
                    // });
                  swal("Registered!", "Annotator: " + inputValue, "success");
                });  
            }
        ...
        function deleteFeature(feature) {
          ...
          if(featureid==undefined || featureid > first_pass_length || first_pass_length==undefined)
          {
        ...
        // new function for deleteFirstPass
        function deleteAllFirstPass() {
          firstpass = vector_data.getSource().getFeatures();

          if(firstpass.length != 0)
          {
              for(i=0; i <= firstpass.length -1 ; i++){
                vector_data.getSource().removeFeature(firstpass[i]); 
                vector_deletions.getSource().addFeature(firstpass[i]);
                //console.info(firstpass[i]);
              } 
              updatecounts();      
          }
        }

        // new function for delete selected region
        function deleteSelectedRegion() {
          firstpass = vector_data.getSource().getFeatures();
          featurearr= vector_edit.getSource().getFeatures();

          if(firstpass.length != 0 || featurearr.length !=0)
          {
              addSelectBoxInteractionForDelete();
              updatecounts();      
          }
        }

        function deleteSelectedRegionOnMarking() {
          featurearr= vector_edit.getSource().getFeatures();

          if(featurearr.length !=0)
          {
              addSelectBoxOnMarkingInteractionForDelete();
              updatecounts();      
          }
        }

        $("#deleteAll_features").click(function(event){
            event.preventDefault();
            var r = confirm("Do you want to delete all first pass:");
            if (r == true){
                deleteAllFirstPass();
            }
        });

        $("#deleteSelectedBox_features").click(function(event){
            event.preventDefault();
            var r = confirm("Do you want to delete selected first pass:");
            if (r == true){
        ...
        $("#deleteSelectedBoxOnMarking_features").click(function(event){
            event.preventDefault();
            var r = confirm("Do you want to delete marked cell from pervious annotators:");
            if (r == true){
                deleteSelectedRegionOnMarking();
            }
        });      
        ...
        function save_data(stat_comp){
            ...
            features_org = vector_data.getSource().getFeatures();

            var del_feat_id = [];
            var del_feat_coordiate = [];       
            var added_features = [];
            var added_features_new_cooridate = [];
            var orginal_update_features = [];
            var orginal_update_features_cooridate = [];
            ...
            //add new geojson file for final output
            // issue with init point [11999, -11999]
            //
            for( var i in features_org){
                if (_.isEqual(features_org[i].getGeometry().j, [11999, -11999]) === false){
                    orginal_update_features.push(new ol.format.GeoJSON().writeFeature(features_org[i]));
                    // sampleArray = features_org[i].getGeometry().j;
                    // sampleArray = sampleArray.map(function(each_element){
                    //     return Number(each_element.toFixed(2));
                    // });
                    orginal_update_features_cooridate.push(features_org[i].getGeometry().j);  
                }                     
            }

            for( var i in features_new){
                //if(features_org.length !=0){
                    orginal_update_features.push(new ol.format.GeoJSON().writeFeature(features_new[i]));
                    // sampleArray = features_new[i].getGeometry().j;
                    // sampleArray = sampleArray.map(function(each_element){
                    //     return Number(each_element.toFixed(2));
                    // });
                    orginal_update_features_cooridate.push(features_new[i].getGeometry().j);
                //}
            }
            //
            //
            // end of adding new geojson file for final output
            ...
            if (stat_comp == 0){
                if (del_feat_coordiate.length != 0 || added_features_new_cooridate.length != 0){
                    if(label=='N'){
                        ...
                        var init_multicell_json =[];
                        init_multicell_json[0]=
                            {
                                'type':'Feature',
                                'id':1,
                                'geometry':{
                                    'type':'MultiPoint',
                                    'coordinates':orginal_update_features_cooridate
                                }
                            }
                        var geojsoncellObject = {"type":"FeatureCollection","features":init_multicell_json}; 
                        rename_slice_no = slice_no.replace('.jp2', '.json');
                        rename_slice_no = rename_slice_no.replace('%26', '&');
                        saveTextAsFile(JSON.stringify(geojsoncellObject), rename_slice_no);
                    }
                }
            }else{
                if(label == 'N'){
                    ...
                    regquery1 = '{"brainno": "' + br_no + '","sliceno":"' + slice_no + '"' + ',"color":"' + color +'","del_id":"' + JSON.stringify(del_feat_id) + '", "add_id": "'+ finalstring + '"}';
                }else{
                    regquery1 = '{"brainno": "' + br_no + '","sliceno":"' + slice_no + '"' + ',"color":"' + color + '","del_id":"' + JSON.stringify(del_feat_id) + '", "add_id": "'+ JSON.stringify(added_features_new_cooridate) + '"}';
                    

                    //output geojson final
                    ...
                    var geojsoncellObject = {"type":"FeatureCollection","features":init_multicell_json};
                    regquery2 = {"brainno": br_no, "sliceno": slice_no, "color": color, "final":JSON.stringify(geojsoncellObject)};
                    //regquery2 = {"brainno": br_no, "sliceno": slice_no, "color": color, "final":JSON.stringify(added_features_new_cooridate)};
                    ////// turf - volume analysis 
                    //var areageojsonpolygon = turf.convex(geojsoncellObject);
                    //regquery3 = {"brainno": br_no, "sliceno": slice_no, "color": color, "final":areageojsonpolygon};

                }

                $.ajax({
                    type : "POST",
                    url  : "/mamo/postnewcellcounts.php",
                    data: {'qry' :regquery1},
                    success :function(result){
                        dirty = false;
                    },
        ...
        function getDeleteID(){
            var returnvalue;
            var regquery = '{"brainno": "'+ br_no +'","sliceno":"'+slice_no + '"' + ',"color":"'+ color +'"}';

            $.ajax({
                type : "POST",
                url  : "http://www.braincircuits.org/mamo/postdeletcellcounts.php",
                data: {'qry' :regquery},
                async: false,
                success :function(result){
                    returnvalue = result;
        ...
        function getAddedID(){
            ...
            $.ajax({
                type : "POST",
                url  : "http://www.braincircuits.org/mamo/postaddcellcounts.php",
                data: {'qry' :regquery},
                async: false,
                success :function(result){
                    if(result =='s'){
        ...
        var onoffSelect = document.getElementById('annotation_onoffswitch');
        var draw;
        var regquery='';
        
        function addSelectBoxInteractionForDelete()
        {
            app.map.removeInteraction(draw);
            firstpass = vector_data.getSource().getFeatures();
            featurearr= vector_edit.getSource().getFeatures();
            ...
            draw.on('drawend',function(event){
                ...
                reg = JSON.parse(payload);
                
                var geojsoncellObject = {"type":"Feature","geometry":reg};
                // first pass selected box deletion
                if(firstpass.length != 0)
                {
                  for(i=0; i <= firstpass.length -1 ; i++){
                    var pt = turf.point(firstpass[i].i.geometry.j);
                    if(turf.booleanPointInPolygon(pt, geojsoncellObject)){
                        vector_data.getSource().removeFeature(firstpass[i]); 
                        vector_deletions.getSource().addFeature(firstpass[i]);
                    }
                  }      
                }
                // marking selected box deletion
                // if(featurearr.length != 0)
                // {
                //   for(i=0; i <= featurearr.length -1 ; i++){
                //     var pt = turf.point(featurearr[i].i.geometry.j);
                //     if(turf.booleanPointInPolygon(pt, geojsoncellObject)){
                //        vector_edit.getSource().removeFeature(featurearr[i]); 
                //        vector_deletions.getSource().addFeature(featurearr[i]);
                //     }
                //   }      
                // }
                updatecounts();
        ...
        function addSelectBoxOnMarkingInteractionForDelete()
        {
            app.map.removeInteraction(draw);
            featurearr= vector_edit.getSource().getFeatures();
        ...
        function drawInjectionSiteInPolygon()
        {
            app.map.removeInteraction(draw);
            firstpass = vector_data.getSource().getFeatures();
            featurearr= vector_edit.getSource().getFeatures();
            ...
            draw.on('drawend',function(event){
                ...
                var geojsoncellObject = {"type":"Feature","geometry":reg};
                //marking selected box deletion
                if(featurearr.length != 0 || firstpass.length !=0)
                {
                  for(i=0; i <= featurearr.length -1 ; i++){
                    var pt = turf.point(featurearr[i].i.geometry.j);
                    if(turf.booleanPointInPolygon(pt, geojsoncellObject)){
        ...
        function show_inj_volume()
        {
            var returnvalue;
            var regquery5 = '{"brainno": "'+ br_no +'","sliceno":"'+slice_no + '"' + ',"color":"'+ color +'"}';

            $.ajax({
                type : "POST",
                url  : "http://braincircuits.org/mamo/postInjVolume.php",
                data: {'qry' :regquery5},
                async: false,
                success :function(result){
                    returnvalue = result; 
        ...
        function show_points()
        {
            ...
            json_file = pid.replace('jp2','json');
            if(color != null){
                color_in_file = '/json' + color + '/';
                decode_file_for_jsonColor = (json_file.length && json_file[0] == '/') ? json_file.slice(1) : json_file; // remove '/' if it's on the first string
                decode_file_for_jsonColor = decode_file_for_jsonColor.split('/').join(color_in_file);
                url_inj_soma_json = hostname + 'celldetection_data/' + decode_file_for_jsonColor;
            }else{
                url_inj_soma_json = hostname + 'celldetection_data/' + json_file;
            }
            ...
            $.ajax({
                ...
                async: false,
                type: 'GET',
                //url:'/ol_cshl_anno.html?brain_id=' + brain_id + '&label=' + label + '&pid=' + app.rft_id,
                url: url_inj_soma_json,
                ...
                success: function (data) {
                    ...
                    if(label =='F' || label =='IHC'){
                        ...
                        // ptarr = jsmesg;
                        // geojsonObject = jsmesg;
                        geojson_data_from_server = jsmesg;
                             
                        ptarr = geojson_data_from_server.features[0].geometry.coordinates;
                        ptarr = ptarr.slice(0, -1);
        ...
        function show_points_marmoset()
        {
            reg = "";
            
            var select_type = 'all';
            var filename = pid.split("/");
            var brainnumber = br_no.replace('m','');
            regquery = '{"brainno": '+ brainnumber +',"sliceno":"'+filename[3]+ '","countall":"'+ select_type + '"}';
            
            $.ajax({
                url:'/mamo/regionquery.php',
                type: 'POST',
        ...
        var source_split = new ol.source.Vector({
            wrapX: false,
            features: (new ol.format.GeoJSON()).readFeatures({'type': 'FeatureCollection', 'features': []})
        });
        var vector_split = new ol.layer.Vector({
            source: source_split,
            style: new ol.style.Style({
              fill: new ol.style.Fill({
                color: 'rgba(255, 255, 255, 0)'
        ...
        var vector_split_highlight = new ol.layer.Vector({
            source: source_split,
            style: new ol.style.Style({
              fill: new ol.style.Fill({
                color: 'rgba(255, 255, 255, 0)'
        ...
        if(inj_volume !=''){
            var source_inj_volume = new ol.source.Vector({
                wrapX: false,
                features: (new ol.format.GeoJSON()).readFeatures(inj_volume)
            });   
            var vector_inj_volume = new ol.layer.Vector({
                source: source_inj_volume,
                style: new ol.style.Style({
                  fill: new ol.style.Fill({
            ...
            app.map.addLayer(vector_inj_volume); 
            vector_inj_volume.setVisible(false); 
        }

        app.map.addLayer(vector_split);  
        app.map.addLayer(vector_split_highlight);

        document.getElementById('intsplittype').onchange = function(){
            vector_split.getSource().clear();
            addSplitInteraction();
            ...
            if(value !='None'){
                ...
                for (var i in json_data_added){
                    if(json_data_added != 0){
                        ...
                        vector_split.getSource().addFeature(json_data_added[i]);
                        json_data_added[i].setStyle(style);
                    }
                }
            }
