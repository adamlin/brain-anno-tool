function create_slider(fid, pid){
    ...
    var colorRanges;
    if(window.location.pathname != "/openlayers/ol7.html"){
    ...
    // range picker for MRI data

    $mri_contrast_range.ionRangeSlider({
        min: 0.1,
        max: 10,
        from: (ini_mri_range == null) ? 1 : ini_mri_range[0],
        grid: true,
        step: 0.1,
        onFinish: function (data) {
            mri_contrast_data = data.from;
            if (ini_mri_range == null){
                app.zoomurl = 'http://mitra.brain.riken.jp:81/fcgi-bin/iipsrv.fcgi?'+ 'CNT='+ mri_contrast_data + '&GAM=' +mri_gamma_data + '&zoomify=/var/www/html/wp-content/uploads/'+fid+'/'+pid+'/';
            }else{
                if(slice_type == 'mri'){
                    app.zoomurl = 'http://mitra.brain.riken.jp:81/fcgi-bin/iipsrv.fcgi?'+ 'CNT='+ mri_contrast_data + '&GAM=' +ini_mri_range[1] + '&zoomify=/var/www/html/wp-content/uploads/'+fid+'/'+pid+'/';
                }
            }

            if (ini_mri_range == null){
                localStorage['mri_range'] = [mri_contrast_data, '1'];
            }else{
                localStorage['mri_range'] = [mri_contrast_data, ini_mri_range[1]];
            }
            if(label == 'mri'){
                //app.zoomurl = 'http://mitra.brain.riken.jp:81/fcgi-bin/iipsrv.fcgi?'+ 'CNT='+mri_contrast_data +'&zoomify=/var/www/html/wp-content/uploads/'+fid+'/'+pid+'/';
                //app.imageLayer.setContrast(gamma_data);
                refresh_layer(app.rft_id);
            }
        },
    });

    $mri_gamma_range.ionRangeSlider({
        min: 0.1,
        max: 2,
        from: (ini_mri_range == null) ? 1 : ini_mri_range[1],
        grid: true,
        step: 0.1,
        onFinish: function (data) {
            mri_gamma_data = data.from;
            if (ini_mri_range == null){
                app.zoomurl = 'http://mitra.brain.riken.jp:81/fcgi-bin/iipsrv.fcgi?'+ 'CNT='+ mri_contrast_data + '&GAM=' +mri_gamma_data + '&zoomify=/var/www/html/wp-content/uploads/'+fid+'/'+pid+'/';
            }else{
                if(slice_type == 'mri'){
                    app.zoomurl = 'http://mitra.brain.riken.jp:81/fcgi-bin/iipsrv.fcgi?'+ 'CNT='+ini_mri_range[0] + '&GAM='+ mri_gamma_data +'&zoomify=/var/www/html/wp-content/uploads/'+fid+'/'+pid+'/';
                }
            }

            if (ini_mri_range == null){
                localStorage['mri_range'] = ['1', mri_gamma_data];
            }else{
                localStorage['mri_range'] = [ini_mri_range[0], mri_gamma_data];
            }
            if(label == 'mri'){
                //console.info('I am MRI and contract is changing here: gamma is: ' + mri_gamma_data);
                //app.zoomurl = 'http://mitra.brain.riken.jp:81/fcgi-bin/iipsrv.fcgi?'+ 'CNT='+mri_gamma_data +'&zoomify=/var/www/html/wp-content/uploads/'+fid+'/'+pid+'/';
                //app.imageLayer.setContrast(gamma_data);
                refresh_layer(app.rft_id);
            }
        },
    ...
    $mri_contrast_range.on("change", mri_contrast_track);
    $mri_gamma_range.on("change", mri_gamma_track);
}

function cellmapping_between_nissl_fluro(){
    if(pid.includes('F')){
        fid = fid.replace('F', 'N');
        pid = pid.replace('F', 'N');
        if(brain_id == '919'){
            pid = pid.replace('--_2', '--_1');
            pid = pid.replace('--_3', '--_2');
        }
    }else{
        fid = fid.replace('N', 'F');
        pid = pid.replace('N', 'F');
    }
    var url = 'http://mitra.brain.riken.jp/wp-content/uploads/'+fid+'/'+pid;
    $.ajax({
        url: url,
        type: 'HEAD',
        error: function () {
            console.info('does not exist');
            swal("Can't find assoicated Nissl", "The file name may not exit in this dataset or may exit in different order");
        },
        success: function () {
            console.info('file exist');
            app.rft_id = 'http://mitra.brain.riken.jp/wp-content/uploads/'+fid+'/'+pid + '&svc.crange=0-255,0-255,0-255';
            refresh_layer(app.rft_id); 
        }
    });
    // app.rft_id = 'http://mitra.brain.riken.jp/wp-content/uploads/'+fid+'/'+pid + '&svc.crange=0-255,0-255,0-255';
...
function refresh_layer(rft_id){
    ...
    }else if(window.location.pathname == "/openlayers/ol6.html"){
        reloadJs("/openlayers/scripts/viewer_ol6.js"); 
    }else if(window.location.pathname == "/openlayers/ol8.html"){
        reloadJs("/openlayers/scripts/viewer_ol8.js"); 
    }else if(window.location.pathname == "/openlayers/ol7.html"){
        reloadJs("/openlayers/scripts/viewer_ol7.js"); 
    }else if(window.location.pathname == "/openlayers/ol_mri.html"){
        reloadJs("/openlayers/scripts/viewer_mri2.js"); 
    }else if(window.location.pathname == "/openlayers/ol4_cross.html"){
        reloadJs("/openlayers/scripts/viewer_ol4_cross.js"); 
    }else{
