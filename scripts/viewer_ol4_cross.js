...
$(function() {
    ...
    jsrc.getImageMetadata(function() {
        ...
        // modify features for annotation
        ...
        var modifyarr = new ol.interaction.Modify({
            features: featuresarr,
            deleteCondition: function(event) {
                return ol.events.condition.shiftKeyOnly(event) &&
                ol.events.condition.singleClick(event);
        ...
        app.map.addInteraction(modifyarr);
            ...
            if (value !== 'None'){
                ...
                draw.on('drawend', function(evt) {
                    ...
                    if(value == 'Anno_Polygon'){
                        var inputValue = ceoSelect.value;
                        if(inputValue !='None'){
                          ...
                          kinks = turf.kinks(turf.simplify(geoj));
                            ...
                            swal({
                              ...
                              text: "Please label annotated region with precise name:",
                              type: "input",
                            ...
                            function(inputValue){
                              ...
                              if(check_input_label_name(inputValue) == 0){
                                swal.showInputError("There is no match label in atlas!");
                                return false
                              }
        ...
        var geoSelect = document.getElementById('geomtype');
        var ceoSelect = document.getElementById('ceomtype'); // new for region annoation on cell mapping required by Bingxing - 23.02.2018 Adam Lin
        var intSelect = document.getElementById('inttype'); 

        geoSelect.onchange = function(){
            app.map.removeInteraction(draw);
            if(geoSelect.value=="Anno_Polygon"){
              $('#ctypesel').css('display','block');
              addMarkingInteraction();
            }else if (geoSelect.value=="Point"){
              $('#ctypesel').css('display','none');
              addMarkingInteraction();  
            }else{
              addMarkingInteraction();  
            }
        ...
        function save_data(stat_comp){
            ...
            for ( var i in features_del){
                ...
                var del_array = features_del[i].getGeometry().j;
                var del_temp_array = [];
                del_temp_array.push([Math.round(del_array[0]), Math.round(del_array[1])]);

                del_feat_coordiate.push(del_temp_array);
            ...
            if (stat_comp == 0){
                if (del_feat_coordiate.length != 0 || added_features_new_cooridate.length != 0){
                    ...
                    for (var i=0; i < added_features.length; i++){
                        ...
                        if (temp_array.geometry.type == 'Polygon'){
                            var label_name = [];
                            var region_temp_array = [];
                            var region_array = temp_array.geometry.coordinates[0];

                            region_array.forEach(function(element){ 
                                region_temp_array.push([Math.round(element[0]), Math.round(element[1])]);
                            })

                            label_name.push('region');
                            label_name.push(temp_array.properties.name);
                            region_temp_array.push(label_name);
                            var temp_string = JSON.stringify(region_temp_array);
                            var final_array_string = temp_string.slice(1, -1);
                        }
                        
                        if (temp_array.geometry.type == 'Point'){
                            var points_on_array = [];
                            var label_name = [];
                            label_name.push('point');
                            label_name.push('');
                            points_on_array.push([Math.round(temp_array.geometry.coordinates[0]),Math.round(temp_array.geometry.coordinates[1])]);
                            points_on_array.push(label_name);
                            var temp_string = JSON.stringify(points_on_array);
                            var final_array_string = temp_string;
                        }
                ...
                if(cellmapping == 1){
                    finalstring = '';
                    var contain_polygon = 0;
                    var contain_point = 0;
                    if(added_features.length !=0){
                        var added_region_point = [];
                        for (var i=0; i < added_features.length ; i++){
                               added_region_point.push(JSON.parse(added_features[i]));
                               if (added_region_point[i].geometry.type == 'Polygon'){
                                    contain_polygon = contain_polygon + 1;
                               }else{
                                    contain_point = contain_point + 1;
                               }
                        }

                        if(contain_point - app.annoated_point.length >= 1){
                            var determine_id_on_features = [];
                            var counting = 0;
                            var count_object = features_new.length - app.annoated_point.length - contain_polygon;
                            for (var i=0; i < features_new.length; i++){
                                determine_id_on_features.push(JSON.parse(added_features[i]));
                                if(determine_id_on_features[i].geometry.type == 'Point' && determine_id_on_features[i].id != undefined){
                                    geoj = JSON.parse((new ol.format.GeoJSON()).writeFeature(features_new[i]));                          
                                    app.annoated_point.push(geoj);

                                    counting = counting + 1;
                                    if (count_object == counting){
                                        break;
                                    }
                                }
                            }
                        ...
                        if(contain_polygon - app.annoated_region.length >= 1){
                            var determine_id_on_features = [];
                            var counting = 0;
                            var count_object = features_new.length - app.annoated_region.length - contain_point;
                            for (var i=0; i < features_new.length; i++){
                                determine_id_on_features.push(JSON.parse(added_features[i]));
                                if(determine_id_on_features[i].geometry.type == 'Polygon' && determine_id_on_features[i].id != undefined){
                                    geoj = JSON.parse((new ol.format.GeoJSON()).writeFeature(features_new[i]));                          
                                    app.annoated_region.push(geoj);
                        ...
                        if (app.annoated_region != 0 && app.annoated_point !=0){
                            finalstring = finalstring + ',';
                        }

                        for (var i=0; i < app.annoated_point.length; i++){
                            var temp_string = JSON.stringify(app.annoated_point[i].geometry.coordinates);
                            // check if it is point or polygon 
                            var on_polygon = temp_string.split(',').length;
                            ...
                            finalstring = finalstring + final_array_string;
                            if (i != app.annoated_point.length - 1){
                                finalstring = finalstring + ',';
                            }
                        }
                    }
                ...
                $.ajax({
                    type : "POST",
                    url  : "/openlayers/postnewcellcounts_cellmapping.php",
                    data: {'qry' :regquery1},
        ...
        function getDeleteID(){
            ...
            $.ajax({
                type : "POST",
                url  : "/openlayers/postdeletcellcounts_cellmapping.php",
                data: {'qry' :regquery},
                async: false,
                success :function(result){
                    returnvalue = result;
        ...
        function getAddedID(){
            ...
            $.ajax({
                type : "POST",
                url  : "/openlayers/postaddcellcounts_cellmapping.php",
                data: {'qry' :regquery},
                async: false,
                success :function(result){
                    if(result =='s'){
                        ...
                        if(label =='N' || label == 'mri'){
                            result = result.replace(/'/g,'"');
                            returnvalue = JSON.parse("[" + result + "]");
                            if (returnvalue[0][0][0] == 'slice_number'){
                                retrived_slice_number = returnvalue[0][0][1];
                                returnvalue.shift();
                                // show message that this annoation was retrived from other closest sections
                                swal({
                                  title: 'Retrived Closest Sections!',
                                  text: 'Section Number: ' + retrived_slice_number,
                                  timer: 1000,
                                  type: 'success'
                                })
                            }
            ...
            $.ajax({
                ...
                data: {'qry' :regquery},
                    ...
                    if (add_json_id != ''){
                        ...
                        }else if (cellmapping == 1){
                            if(isNaN(add_json_id[0][add_json_id[0].length - 1][0]) == true || (add_json_id[0][add_json_id[0].length - 1][0]) != ''){
                                for (var i = 1; i < 1 + add_json_id.length; i++){
                                    if (add_json_id[i-1].length > 2){
                                        var pro_name = add_json_id[i-1][add_json_id[i-1].length - 1][0];
                                        var color_code = retrive_label_color_code(pro_name);
                                        var name_code = retrive_label_name(pro_name);
                                        var pro_acronym = add_json_id[i-1][add_json_id[i-1].length - 1][1];
                                        add_json_id[i-1].pop();
                                        init_cell_add_json[i -1]=
                                            {
                                                "type":"Feature",
                                                "id":ptarr.length + i,
                                                "properties":{
                                        ...
                                        //app.annoated_region.push(init_cell_add_json[i-1]);        
                                    }else if (add_json_id[i-1].length == 2){
                                        //add_json_id[i-1].pop();
                                        init_cell_add_json[i -1]=
                                        {
                                            "type":"Feature",
                                        ...
                                        //app.annoated_point.push(init_cell_add_json[i-1]); 
                                    }
                                }
                            }else{
                               for (var i = ptarr.length + 1; i < ptarr.length + 1 + add_json_id.length; i++){
                        ...
                        for (var i in json_data_added){
                            if(json_data_added != 0){
                                if (geojsonAddedObject.features[i].geometry.type == 'Polygon'){
                                    var rgb_color = 'rgba(' + geojsonAddedObject.features[i].properties.acronym + ' 0.5)';
                                ...
                                if (geojsonAddedObject.features[i].geometry.type == 'Point'){
                                    style = new ol.style.Style({
                                      fill: new ol.style.Fill({
