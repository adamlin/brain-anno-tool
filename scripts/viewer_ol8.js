    ...
    jsrc.getImageMetadata(function() {
        ...
        function save_data(stat_comp){
            ...
            for ( var i in features_del){
                ...
                // round the output - modified by Adam Lin 21 March 2018 - requesed by Paul M. 
                //del_feat_coordiate.push(features_del[i].getGeometry().j);

                var del_array = features_del[i].getGeometry().j;
                del_feat_coordiate.push([Math.round(del_array[0]), Math.round(del_array[1])]);
            }
            ...
            if (stat_comp == 0){
                if (del_feat_coordiate.length != 0 || added_features_new_cooridate.length != 0){
                    if(label=='N'){
                        ...
                        for (var i=0; i < added_features.length; i++){
                            ...
                            // round the output - requesed by Paul M.
                            var add_temp_array = temp_array.geometry.coordinates[0];
                            var add_temp_around_obj=[];
                            for (var j = 0; j <= add_temp_array.length - 1; j++) {
                                if(j == add_temp_array.length - 1 && temp_array.properties.name != ''){
                                    add_temp_around_obj[j] = ([temp_array.properties.name, '']);
                                }else{
                                    add_temp_around_obj[j] = ([Math.round(add_temp_array[j][0]), Math.round(add_temp_array[j][1])]);
                                }
                            }
                            var temp_string = JSON.stringify(add_temp_around_obj);
                ...
                if(label == 'N'){
                    ...
                    for (var i=0; i < app.annoated_region.length; i++){
                        ...
                        // round the output - requesed by Paul M.
                        var add_temp_array = app.annoated_region[i].geometry.coordinates[0];
                        var add_temp_around_obj=[];
                        for (var j = 0; j <= add_temp_array.length - 1; j++) {
                            if(j == add_temp_array.length - 1){
                                add_temp_around_obj[j] = ([add_temp_array[j][0], add_temp_array[j][1]]);
                            }else{
                                add_temp_around_obj[j] = ([Math.round(add_temp_array[j][0]), Math.round(add_temp_array[j][1])]);
                            }
                        }
        ...
        var stroke_p = new ol.style.Stroke({
         ...
         var stroke_b = new ol.style.Stroke({
            color: '#ede409',
            width: 1.5
        });
        var styles_p = [
                    ...
                    if (add_json_id != ''){
                        ...
                        for (var i in json_data_added){
                            if(json_data_added != 0){
                                if(json_data_added[i].getProperties().acronym == undefined){
                                    var rgb_color = 'rgba(255,255,0)';
                                }else{
                                ...
                                if(json_data_added[i].getProperties().acronym == undefined){
                                    json_data_added[i].setStyle(styles_b);
                                }else{
